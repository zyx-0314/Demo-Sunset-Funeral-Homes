...
<article
    ...
    data-id="<?= esc($service->id ?? '', 'attr') ?>"
    data-category="<?= esc($service->category ?? '', 'attr') ?>"
    data-cost="<?= esc($service->cost ?? '', 'attr') ?>"
    data-created="<?= esc($service->created_at ?? '', 'attr') ?>">
    ...
    <?php if ($service->image): ?>
        <img
            ...
            src="<?= esc($service->image, 'attr') ?>"
            alt="<?= esc($service->title ?? 'Service image', 'attr') ?>">
    <?php else: ?>
        <div class="flex justify-center items-center bg-slate-100 w-full h-44">
            <img
                src="<?= esc($service->banner_image ? "/" . $service->banner_image : 'https://images.unsplash.com/photo-1519681393784-d120267933ba?auto=format&fit=crop&w=1400&q=80', 'attr') ?>"
                alt="<?= esc($service->title ?? 'no image', 'attr') ?>">
        </div>
    <?php endif; ?>
    <div class="flex flex-col flex-1 mt-8 py-4">
        <h3
            class="bg-white mb-1 px-4 pt-4 min-h-[4rem] overflow-hidden font-medium text-slate-900 text-lg line-clamp-2"
